...
from src.utils import load_data, search_movies
from src.train import train_knn
from src.validation import recommend_similar_movies

# ---- Load data and train model
movies, ratings, movie_user_matrix, ratings_sparse = load_data()
knn_model = train_knn(ratings_sparse)
...
def search(query: str, limit: int = 20):
    items = search_movies(movies, query, available_titles=AVAILABLE_TITLES).head(limit)
    return {"items": [{"title": r.title, "genres": r.genres} for r in items.itertuples(index=False)]}

@app.post("/recommend")
def recommend(body: RecommendBody):
    recs = recommend_similar_movies(body.title, movie_user_matrix, ratings_sparse, knn_model, movies, body.top_n)
    return {"items": recs, "base": body.title}
...
def recommend_batch(body: RecommendBatchBody):
    ...
    for t in body.titles:
        try:
            for r in recommend_similar_movies(t, movie_user_matrix, ratings_sparse, knn_model, movies, body.top_n):
                pool[r["title"]] = max(pool.get(r["title"], 0.0), r["similarity"])
        except HTTPException:
            continue

    items = [{"title": k, "similarity": round(v, 4), "genres": movies.loc[movies["title"] == k, "genres"].values[0]}
             for k, v in pool.items() if k not in set(body.titles)]
    items.sort(key=lambda x: x["similarity"], reverse=True)
    return {"items": items[:body.top_n], "base": body.titles}